...
function AddClub() {
  ...
  const pageRefs = useRef(
    allPossiblePages.reduce((refs, page) => {
    ...
    const fetchData = async () => {
      ...
      const response = await fetch(
        ...
        .then((data) => {
          ...
          const selectedFile = new File([buffer], "Previous Image.png", {
            type: "image/png",
          });
          // title
  ...
  const addPage = (pageToAdd) => {
    setAllCurrentPages((prevPages) => {
      const insertIndex = prevPages.findIndex(
        (page) =>
          allPossiblePages.indexOf(page) > allPossiblePages.indexOf(pageToAdd)
      );
      if (insertIndex === -1) {
      ...
      <div className="pop-up-container">
        ...
        <div className="pop-up-content">
          ...
          <div className="pop-up-content-description">
            ...
            <a href="mailto:communityalis@gmail.com">
              {" "}
              communityalis@gmail.com{" "}
            </a>
          </div>
        </div>
      </div>
      ...
      <form method="POST" className="service-container" id="form">
        ...
        <div className="service-navbar">
          {allCurrentPages.map((page) => (
            <div
              className="service-navbar-text"
              key={page}
              id={page === "OverviewPage" ? "" : `remove${page}`}
            >
              <a
                href="#"
                className="service-navbar-link"
                ref={pageRefs.current[page]}
                onClick={() => changeVisibility(page)}
              >
                {page}
              </a>
              {removablePages.includes(page) && (
                <i
                  className="fa-solid fa-circle-xmark"
                  id="remove-service-navbar-text"
        ...
        {activePage === "Overview" && (
          <OverviewPage
            ...
            setFormData={setOverviewFormData}
            editMode={true}
          />
        )}
        {activePage === "Contacts" && (
          <ContactsPage
            ...
            setFormData={setContactsFormData}
          />
        )}
        ...
        {activePage === "Sign-Up" && (
          <SignUpPage
            key="SignUpPage"
            handleShowPromptChange={handleShowPromptChange}
            editMode={true}
            mainInfo={{ title: titleValue }}
            allFormData={{
